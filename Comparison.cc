...
CNF::CNF() {
	numAnds =0; //so that compare will by default return 1
}
...
/* OrderMaker::numOfAtts()
 * This will return the number of attributes in order maker
 * @ Vineet Garg, 21 March, 2011
 */
...
/* OrderMaker::attsList()
 * This will return the list of attributes in order maker
 * @Vineet Garg, 21 March, 2011
 */
...
bool CNF::QueryMaker(OrderMaker &sortOrder, OrderMaker &query) {
    ...
    query.numAtts =0;
    //traverse the sort order
    for (int count = 0; count < sortOrder.numAtts; count++) {

        attributeFound = false;
        currentAtt = sortOrder.whichAtts[count];

        //search for whichAtts in CNF
        for (i = 0; i < numAnds; i++) {

            //there should be only one expression in the orLens for this attribute to be considered in the "query" orderMaker
            if (orLens[i] == 1) {
                if (orList[i][j].op == Equals && orList[i][j].whichAtt1 == currentAtt) {
                    query.whichAtts[query.numAtts] = orList[i][j].whichAtt2;
                    query.whichTypes[query.numAtts] = orList[i][j].attType;
                    query.numAtts++;
                    attributeFound = true;
                    //getchar();
                    break;
                }
            } else //move to next And
            {
                //cout<<"Skip the multiple subexpression\n";
                continue;
            }
        }
        // if no check found for last attribute then no need to check rest of the order attributes
