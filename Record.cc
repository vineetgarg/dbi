...
/* Write_To_File(FILE*, Schema*)
 * Instead of printing to console write the records to file
 * @Vineet Garg
 * 17 March, 2010
 */
void Record :: Write_To_File (FILE *opFile, Schema *mySchema) {

	//cout<<"Record::Write_To_File called";
	int n = mySchema->GetNumAtts();
	Attribute *atts = mySchema->GetAtts();
	...
	for (int i = 0; i < n; i++) {
		...
		if (atts[i].myType == Int) {
			int *myInt = (int *) &(bits[pointer]);
			//cout << *myInt;
			fprintf(opFile, "%d", *myInt);

		// then is a double
		} else if (atts[i].myType == Double) {
			double *myDouble = (double *) &(bits[pointer]);
			//cout << *myDouble;	
			fprintf(opFile, "%Lf", myDouble);

		// then is a character string
		} else if (atts[i].myType == String) {
			char *myString = (char *) &(bits[pointer]);
			//cout << myString;	
			fprintf(opFile, "%s", myString);
 ...
 * @Vineet Garg, 20 March, 2011 */
int Record :: numOfAttInRecord(){
	//return ((bits[1]/sizeof(int)) - 1);
	int att2 = *((int*)bits +1);
